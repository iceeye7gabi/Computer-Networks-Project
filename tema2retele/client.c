...
int main (int argc, char *argv[])
{
  ...
  struct sockaddr_in server;	// structura folosita pentru conectare 
  		// mesajul trimis
  int nr=0;
  char message[100];char logare[100];
  char nickname[100];char password[100];int type;
  ...
  while(1){
  /* citirea mesajului */
  printf ("[client]Introduceti o comanda: "); fflush (stdout);
  scanf ("%s", &message); fflush (stdin);
  ...
  if(strcmp(message,"quit")==0) {
  	...
  	printf("Introduceti nickname-ul: ");fflush(stdout);scanf("%s",&nickname);fflush(stdin);
  	write (socket_descriptor, &nickname,sizeof(nickname));
  	printf("Introduceti parola: ");fflush(stdout);scanf("%s",&password);fflush(stdin);
  	write (socket_descriptor, &password,sizeof(password));
  	read(socket_descriptor,&logare,sizeof(logare));
  	printf("%s\n",logare);
  }
  else if(strcmp(message,"Command not found!")==0) {printf(red_color "Comanda nu exista!\n" white_color); continue;}
